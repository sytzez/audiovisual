...
var pixelArray = new Uint8Array(1024 * audioScale * 4);
var scanPos = 0;
var scanDir = 0;
var scanDirY = 1;
var pswitch = false;
...
(function loop() {
  ...
  if (paused) return;
	...
	gl.readPixels(0, scanPos, 1024, audioScale, gl.RGBA, gl.UNSIGNED_BYTE, pixelArray);
...
aproc.onaudioprocess = function(e) {
	...
	for (var i = 0; i < bufsize; i++) {
		...
		left[scanDir==1 ? bufsize - 1 - i : i] = (pixelArray[i * 4 * audioScale] + pixelArray[i * 4 * audioScale + 2]) / 256.0 / 2.0;
		right[scanDir==1 ? bufsize - 1 - i : i] = (pixelArray[i * 4 * audioScale + 1] + pixelArray[i * 4 * audioScale + 2]) / 256.0 / 2.0;
		//*/
		/*
		lp += pixelArray[i * 4 * audioScale] * 0.001;
		rp += pixelArray[i * 4  * audioScale + 1] * 0.001;
		left[scanDir==1 ? bufsize - 1 - i : i] = Math.sin(lp) * 0.5 + pixelArray[i * 4 * audioScale] * 0.001;
		right[scanDir==1 ? bufsize - 1 - i : i] = Math.sin(rp) * 0.5 + pixelArray[i * 4  * audioScale + 1] * 0.001;
		//*/
	}
